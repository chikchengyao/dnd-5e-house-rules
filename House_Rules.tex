...
Instead of preparing a list of spells, you prepare spells directly into spell slots. Whenever you recover a spell slot, choose a spell to occupy that slot. You can use that slot only to cast that spell, unless you take the Reprepare action (below) to change the occupying spell.
...
\noindent This cleric can cast Bless once (and only once) at 1st level, expending that spell. Her remaining prepared spells will then be:
...
\noindent Because she can cast spells only at the level they were prepared, she can cast \textit{Guiding Bolt} only at 1st level, and \textit{Inflict Wounds} only at 2nd level.
\end{DndComment}

\subsubsection{Action: Reprepare}
You have a number of uses of Reprepare equal to your level, recovering all expended uses on a long rest. You can reprepare a spell by using an action to Reprepare for a number of consecutive turns equal to the spell's level. 
